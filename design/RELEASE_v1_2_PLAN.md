...
| P4 | Serial/modem transport and file transfers | Partial | Config, file-area schema, repository APIs, caller-session transport-generic helpers, XMODEM-CRC fallback, ZMODEM frame primitives, ByteTransport-Transport bridge for protocol integration, and path sanitization utilities exist; physical serial/modem transport, full ZMODEM send/receive state machines, and caller file-area workflows are not implemented. |
